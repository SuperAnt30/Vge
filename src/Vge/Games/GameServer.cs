...
namespace Vge.Games
{
    ...
    public class GameServer
    {
        ...
        private void _SocketServer_UserJoined(object sender, PacketStringEventArgs e)
        {
            ...
            if (_flagInLoop)
            {
                ...
                ResponsePacket(e.Side, new PacketS02LoadingGame(Ce.Blocks.BlockAlias, 
                    ...
                    Ce.BlocksEntity.BlockEntityAlias, Ce.EntitiesFX.EntitiesFXAlias));
            }
            else
            {
        ...
        private string _ToStringDebugTps()
        {
            ...
            return string.Format(ChatStyle.Bolb + "[Server]:" 
                + ChatStyle.Reset
                + " {0:0.00} tps {1:0.00} ms Rx {2} Tx {3} Tick {4} Time {5:0.0} s {6}"
